...
### 2.2 CLI（Web と生成ロジック共有）

Web MVP と並行して npm CLI（`chobitfile`）を提供する。詳細は [CLI 設計検討](./cli-design.md)。

| 項目 | 内容 |
|---|---|
| 形式・サイズ厳密性 | Web と同じ生成器 |
| 上限 | Web と同じ（20 MB + 1） |
| 任意サイズ | CLI のみ（`--size` / `--bytes`）。Web UI はプリセットのまま |
| 一括生成 | 含めない |
| デフォルト | `png` / `1mb` / `exact`（Web と同じ） |
